...
<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <StatCard title="Clôturés" :value="getStatusCount('closed')" icon="check-circle" color="gray" />
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="card">
        <h3 class="text-lg font-semibold mb-4">Incidents par statut</h3>
        <IncidentChart v-if="stats.incidents_by_status" type="doughnut" :data="incidentsByStatusChart" />
      </div>

      <div class="card">
        <h3 class="text-lg font-semibold mb-4">Incidents par priorité</h3>
        <IncidentChart v-if="stats.incidents_by_priority" type="doughnut" :data="incidentsByPriorityChart" />
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="card">
        <h3 class="text-lg font-semibold mb-4">Top entreprises déclarant le plus d'incidents</h3>
        <div v-if="stats.incidents_by_company?.length" class="space-y-3">
          <div v-for="(entry, index) in stats.incidents_by_company.slice(0, 5)" :key="entry.company_id"
            class="flex justify-between items-center">
            <span class="text-sm">{{ index + 1 }}. {{ entry.company?.company_name }}</span>
            <span class="text-sm font-semibold">{{ entry.count }} incidents</span>
          </div>
        </div>
        <EmptyState v-else title="Aucune donnée" message="Aucune entreprise n'a encore déclaré d'incident." />
      </div>
    </div>
    <div class="card">
